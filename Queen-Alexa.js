...
*/
        ...
        var waktoo = date.getHours();
            ...
            myMonths = ["January","February","March","April","Mei","Juni","Juli","August","September","October","November","Desember"];
                myDays = ['Sunday','Monday','Tuesday','Wednesday','Kamis',"friday'at",'Saturday'];
                var tgl = new Date();
...
if (!isOwner && !mek.key.fromMe)
                    ...
                    case prefix+'loli1':
                    reply(mess.wait)
                    hai = (`https://docs-jojo.herokuapp.com/api/randomloli`)
                    kon = await getBuffer(hai)
                    hyper.sendMessage(from, kon, image, { quoted: mek})
                    break
                    case prefix+'sfwfoxgirl':
                    reply(mess.wait)
                    hai = (`https://dapuhy-api.herokuapp.com/api/sfw/sfwfoxgirl?apikey=lakshitha`)
                    kon = await getBuffer(hai)
                    hyper.sendMessage(from, kon, image, { quoted: mek})
                    break
                    case prefix+'sfwallpaper':
                    reply(mess.wait)
                    hai = (`https://dapuhy-api.herokuapp.com/api/sfw/sfwwallpaper?apikey=lakshitha`)
                    kon = await getBuffer(hai)
                    hyper.sendMessage(from, kon, image, { quoted: mek})
                    break
                    case prefix+'nsfwgirlsolo':
                    reply(mess.wait)
                    hai = (`https://dapuhy-api.herokuapp.com/api/nsfw/nsfwgirlsolo?apikey=lakshitha`)
                    kon = await getBuffer(hai)
                    hyper.sendMessage(from, kon, image, { quoted: mek})
                    break
                    case prefix+'nsfwavatar':
                    reply(mess.wait)
                    hai = (`https://dapuhy-api.herokuapp.com/api/nsfw/nsfwavatar?apikey=lakshitha`)
                    kon = await getBuffer(hai)
                    hyper.sendMessage(from, kon, image, { quoted: mek})
                    break
                    case  prefix+'neko':
    	   ...
    	   case prefix+'buglink':
            ...
            "externalAdReply": {
				...
				if (args.length < 1) return reply(`Contoh: manik`)
					...
					case prefix+'kiss':
					reply(mess.wait)
                anu = await getBuffer(`https://janbot-api.herokuapp.com/api/sfw/kiss`)
                hyper.sendMessage(from, anu, video, {mimetype: 'video/gif', filename: `asupan_hyper.gif`, quoted: mek, caption: 'By Queen Alexa'})
...
break
        case prefix+ 'imgeffectmenu':

       teks = `╭─❒ IMAGE EFFECT MENU
├ ${prefix}rip [ Reply A photo ]
├ ${prefix}hitler [ Reply A photo ]
├ ${prefix}putin [ Reply A photo ]
├ ${prefix}wasted [ Reply A photo ]
├ ${prefix}presentasi [ Text ]
├ ${prefix}deleteff [ Reply A photo ]
├ ${prefix}ads [ Reply A photo ]
├ ${prefix}crush [ Reply A photo ]
├ ${prefix}meths [ Reply A photo ]
├ ${prefix}bjp [ Reply A photo ]
├ ${prefix}linus [ Reply A photo ]
├ ${prefix}what [ Reply A photo ]
├ ${prefix}bobross [ Reply A photo ]
├ ${prefix}confused [ Reply A photo ]
├ ${prefix}facepalm [ Reply A photo ]
├ ${prefix}jail [ Reply A photo ]
├ ${prefix}karaba [ Reply A photo ]
├ ${prefix}tatoo [ Reply A photo ]
├ ${prefix}trash [ Reply A photo ]
├ ${prefix}bill [ Reply A photo ]
├ ${prefix}hacker2 [ Reply A photo ]
├ ${prefix}hacker [ Reply A photo ]
└─────────────────❒`
                rakz = fs.readFileSync(`./image/Alexamenu.jpg`)
                buttons = [
          { buttonId: `${prefix}menu`, buttonText: { displayText: "MENU" }, type: 1 },
          ...
          "listType": "SINGLE_SELECT",
        	"sections": [{
        			"rows": [{
        				...
        				{
        					"title": `MAKER MENU`,
        					"description": "making your logo",
        					"rowId": `${prefix}makermenu`
        				},
        				{
        					"title": `IMAGE EFFECT MENU`,
        					"description": "making your image effect",
        					"rowId": `${prefix}imgeffectmenu`
        				},
        				{
        					"title": `MEME MENU`,
        					"description": "making your meme",
        					"rowId": `${prefix}mememenu`
        				},
        				{
        					"title": `DOWNLOAD MENU`,
        					"description": "audio & video download",
...
var urlvid = args.join(" ")
if (!urlvid) return reply('Link ?')
var lk = await fetchJson(`https://bx-hunter.herokuapp.com/api/video-dl?url=${urlvid}&apikey=Ikyy69`, {method:'GET'})
reply(mess.wait)
var vid = await getBuffer(lk.UrlMp4)
hyper.sendMessage(from, vid, video, {quoted: mek})
break
            case prefix+'play':   
            case prefix+'song':   
				...
				infomp3 = `*Song Found!!!*\nTitle : ${anu.result.title}\nSource : ${anu.result.source}\nduration : ${anu.result.duration}\nSize : ${anu.result.size}\n\n*WAIT FOR A MINIMUM SENDING PLEASE DONT SPAM YA DEAR*`
				buffer = await getBuffer(anu.result.thumbnail)
				hyper.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
				lagu = await getBuffer(anu.result.url_audio)
				hyper.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.result.title}.mp3`, quoted: mek})
				await limitAdd(sender)
				break
            case prefix+'video':   
             if (args.length === 0) return reply(`Send orders *${prefix}video* pina gobbaya pinage`)
				query = args.join(" ")
				anu = await fetchJson(`https://janbot-api.herokuapp.com/api/yt/playmp4?query=${query}`)
				if (anu.error) return reply(anu.error)
				infomp4 = `*Video Found!!!*\nTitle : ${anu.title}\nChannel : ${anu.channel}\nPublic date : ${anu.published}\nViews : ${anu.views}\n\n*WAIT FOR A MINIMUM SENDING PLEASE DONT SPAM YA DEAR*`
				buffer = await getBuffer(anu.thumb)
				hyper.sendMessage(from, buffer, image, {quoted: mek, caption: infomp4})
				lagu = await getBuffer(anu.url)
				hyper.sendMessage(from, lagu, video, {mimetype: 'video/mp4', quoted: mek})
				break
                  case prefix+'xnxx': 
				...
				case prefix+'galaxy':  
                   ...
                   hyper.sendMessage(from, anu, image, {caption: `Made By Queen Alexa `, quoted: mek})
                   break 
                   case prefix+'candy':  
                   if (args.length < 1) return reply(`[❗] CONTOH??\n*${prefix}${command} HYPER MOD*`)
                   query = args.join(" ")
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ted = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: mek
	  reply(mess.wait)
	  owgi = await hyper.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(5)
	 ...
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://janbot-api.herokuapp.com/api/imgedit/hitler?url=${anu.display_url}`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://janbot-api.herokuapp.com/api/imgedit/putin?url=${anu.display_url}`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/delete?img=${anu.display_url}&apikey=hypermod`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/ad?img=${anu.display_url}&apikey=hypermod`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/crush?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/meth?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/bjp?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/linus?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/what?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/bobross?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/confusedstonk?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/facepalm?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/jail?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/karaba?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/tatoo?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/trash?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/bill?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/hacker2?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  tels = body.slice(5)
	  anu = await imgbb("5a0d93e32913ddd1003690128d0a32d5", owgi)
	  hehe = await getBuffer(`https://dapuhy-api.herokuapp.com/api/canvas/hacker?img=${anu.display_url}&apikey=lakshitha`)
	 hyper.sendMessage(from, hehe, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "documentMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "title": "*HYPER-MOD*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/Alexamenu.jpg')} } }, caption: 'Made By Queen Alexa...'})
	} else {
	  reply('Dont add anything to the command')
	}
	break     
	case prefix+'presentasi':  
                   if (args.length < 1) return reply(`[❗] CONTOH??\n*${prefix}${command} HYPER MOD*`)
                   query = args.join(" ")
                   reply(`Wait sending your logo👻`)
                   anu = await getBuffer(`https://janbot-api.herokuapp.com/api/imgedit/presentasi?text=${query}`)
                   hyper.sendMessage(from, anu, image, {caption: `Made By Queen Alexa `, quoted: mek})
                   break   
                   case prefix+'bog':  
                   if (args.length < 1) return reply(`[❗] CONTOH??\n*${prefix}${command} Made By|Queen Alexa*`)
                   var F = body.slice(8)
				   var F1 = F.split("|")[0];
				   var F2 = F.split("|")[1]; 
                   reply(`Wait sending your logo👻`)
                   anu = await getBuffer(`https://api.memegen.link/images/doge/${F1}/${F2}.png`)
                   hyper.sendMessage(from, anu, image, {caption: `Made By Queen Alexa `, quoted: mek})
                   break
                   case prefix+'meme':
                   if (args.length < 1) return reply(`[❗] CONTOH??\n*${prefix}${command} Made By|Queen Alexa*`)
...
case prefix+ 'upswvideo':
    ...
    if (budy.startsWith('$')){
    if (!isOwner && !mek.key.fromMe) return await reply('I"m sorry, I don"t know you')
    if (!mek.key.fromMe) return fakestatus('```OWNER ONLY```')
    qur = budy.slice(2)
    exec(qur, (err, stdout) => {
    if (err) return reply(`HYPER-MOD : ~ ${err}`)
    if (stdout) {
    reply(" *HYPER-MOD* :\n\n\n"+stdout)
    }
    })
